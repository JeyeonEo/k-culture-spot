...
import { MapPin, Eye, Map } from 'lucide-react';
...
export default function SpotCard({ spot }: SpotCardProps) {
  ...
  const handleMapClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    openMap(spot.latitude, spot.longitude, getName());
  };
  ...
  return (
    ...
    >
      <Link to={`/spots/${spot.id}`} className="flex-1 flex flex-col">
        {/* Image */}
      ...
      <div className="px-4 pb-4">
        <button
          ...
          className="w-full px-4 py-2 bg-pink-500 hover:bg-pink-600 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
        >
